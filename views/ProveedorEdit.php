...
loadjs.ready("head", function () {
    ...
    fproveedoredit.lists.CodRegion = <?= $Page->CodRegion->toClientList($Page) ?>;
    fproveedoredit.lists.Vigente = <?= $Page->Vigente->toClientList($Page) ?>;
    fproveedoredit.lists.tipo_transferencia = <?= $Page->tipo_transferencia->toClientList($Page) ?>;
...
<?php if ($Page->CodProveedor->Visible) { // CodProveedor ?>
    <div id="r_CodProveedor" class="form-group row">
        <label id="elh_proveedor_CodProveedor" class="<?= $Page->LeftColumnClass ?>"><?= $Page->CodProveedor->caption() ?><?= $Page->CodProveedor->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->CodProveedor->getDisplayValue($Page->CodProveedor->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="proveedor" data-field="x_CodProveedor" data-hidden="1" name="x_CodProveedor" id="x_CodProveedor" value="<?= HtmlEncode($Page->CodProveedor->CurrentValue) ?>">
<?php } else { ?>
<span id="el_proveedor_CodProveedor">
<span<?= $Page->CodProveedor->viewAttributes() ?>>
...
<input type="hidden" data-table="proveedor" data-field="x_CodProveedor" data-hidden="1" name="x_CodProveedor" id="x_CodProveedor" value="<?= HtmlEncode($Page->CodProveedor->FormValue) ?>">
<?php } ?>
</div></div>
    </div>
<?php } ?>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->Vigente->isInvalidClass() ?>" data-table="proveedor" data-field="x_Vigente" name="x_Vigente[]" id="x_Vigente_792865" value="1"<?= ConvertToBool($Page->Vigente->CurrentValue) ? " checked" : "" ?><?= $Page->Vigente->editAttributes() ?> aria-describedby="x_Vigente_help">
    <label class="custom-control-label" for="x_Vigente_792865"></label>
</div>
