...
loadjs.ready("head", function () {
    ...
    fcontratoadd = currentForm = new ew.Form("fcontratoadd", "add");
    ...
    // Form_CustomValidate
    fcontratoadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcontratoadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fcontratoadd.lists.MotivoBaja = <?= $Page->MotivoBaja->toClientList($Page) ?>;
    fcontratoadd.lists.Vigente = <?= $Page->Vigente->toClientList($Page) ?>;
    fcontratoadd.lists.CodServicio = <?= $Page->CodServicio->toClientList($Page) ?>;
    fcontratoadd.lists.CodProveedor = <?= $Page->CodProveedor->toClientList($Page) ?>;
    loadjs.done("fcontratoadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fcontratoadd" id="fcontratoadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["fcontratoadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fcontratoadd", "x_Fecha", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fcontratoadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fcontratoadd", "x_Vencimiento", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->CodigoInterno->Visible) { // CodigoInterno ?>
    <div id="r_CodigoInterno" class="form-group row">
        <label id="elh_contrato_CodigoInterno" class="<?= $Page->LeftColumnClass ?>"><?= $Page->CodigoInterno->caption() ?><?= $Page->CodigoInterno->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->CodigoInterno->cellAttributes() ?>>
<?php if (!$Page->isConfirm()) { ?>
<span id="el_contrato_CodigoInterno">
<input type="<?= $Page->CodigoInterno->getInputTextType() ?>" data-table="contrato" data-field="x_CodigoInterno" name="x_CodigoInterno" id="x_CodigoInterno" size="30" maxlength="20" placeholder="<?= HtmlEncode($Page->CodigoInterno->getPlaceHolder()) ?>" value="<?= $Page->CodigoInterno->EditValue ?>"<?= $Page->CodigoInterno->editAttributes() ?> aria-describedby="x_CodigoInterno_help">
<?= $Page->CodigoInterno->getCustomMessage() ?>
<div class="invalid-feedback"><?= $Page->CodigoInterno->getErrorMessage() ?></div>
</span>
<?php } else { ?>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->Vigente->isInvalidClass() ?>" data-table="contrato" data-field="x_Vigente" name="x_Vigente[]" id="x_Vigente_946475" value="1"<?= ConvertToBool($Page->Vigente->CurrentValue) ? " checked" : "" ?><?= $Page->Vigente->editAttributes() ?> aria-describedby="x_Vigente_help">
    <label class="custom-control-label" for="x_Vigente_946475"></label>
</div>
