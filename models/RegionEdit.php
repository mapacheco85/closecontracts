...
class RegionEdit extends Region
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("CodRegion") ?? Key(0) ?? Route(2)) !== null) {
                $this->CodRegion->setQueryStringValue($keyValue);
                $this->CodRegion->setOldValue($this->CodRegion->QueryStringValue);
            } elseif (Post("CodRegion") !== null) {
                $this->CodRegion->setFormValue(Post("CodRegion"));
                $this->CodRegion->setOldValue($this->CodRegion->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("CodRegion") ?? Route("CodRegion")) !== null) {
                    $this->CodRegion->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->CodRegion->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("regionlist"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = "regionlist";
                if (GetPageName($returnUrl) == "regionlist") {
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash .= GetFieldHash($row['Descripcion']); // Descripcion
        return md5($hash);
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("regionlist"), "", $this->TableVar, true);
        $pageId = "edit";
