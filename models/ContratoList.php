...
class ContratoList extends Contrato
{
    ...
    public function __construct()
    {
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fcontratolistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->Vigente->AdvancedSearch->toJson(), ","); // Field Vigente
        $filterList = Concat($filterList, $this->CodServicio->AdvancedSearch->toJson(), ","); // Field CodServicio
        $filterList = Concat($filterList, $this->CodProveedor->AdvancedSearch->toJson(), ","); // Field CodProveedor
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fcontratolistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Vigente
        $this->Vigente->AdvancedSearch->SearchValue = @$filter["x_Vigente"];
        $this->Vigente->AdvancedSearch->SearchOperator = @$filter["z_Vigente"];
        $this->Vigente->AdvancedSearch->SearchCondition = @$filter["v_Vigente"];
        $this->Vigente->AdvancedSearch->SearchValue2 = @$filter["y_Vigente"];
        $this->Vigente->AdvancedSearch->SearchOperator2 = @$filter["w_Vigente"];
        $this->Vigente->AdvancedSearch->save();

        // Field CodServicio
        $this->CodServicio->AdvancedSearch->SearchValue = @$filter["x_CodServicio"];
        $this->CodServicio->AdvancedSearch->SearchOperator = @$filter["z_CodServicio"];
        $this->CodServicio->AdvancedSearch->SearchCondition = @$filter["v_CodServicio"];
        $this->CodServicio->AdvancedSearch->SearchValue2 = @$filter["y_CodServicio"];
        $this->CodServicio->AdvancedSearch->SearchOperator2 = @$filter["w_CodServicio"];
        $this->CodServicio->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->Vigente); // Vigente
            $this->updateSort($this->CodServicio); // CodServicio
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->Vigente->setSort("");
                $this->CodServicio->setSort("");
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canEdit()) {
                if (IsMobile()) {
                    ...
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"contrato\" data-caption=\"" . $editcaption . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SaveBtn',url:'" . HtmlEncode(GetUrl($this->EditUrl)) . "'});\">" . $Language->phrase("EditLink") . "</a>";
                }
            } else {
                $opt->Body = "";
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if (IsMobile()) {
            ...
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"contrato\" data-caption=\"" . $addcaption . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'AddBtn',url:'" . HtmlEncode(GetUrl($this->AddUrl)) . "'});\">" . $Language->phrase("AddLink") . "</a>";
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcontratolistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcontratolistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fcontratolist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fcontratolistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
