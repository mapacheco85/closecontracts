...
class DocumentoSearch extends Documento
{
    ...
    // Page object name
    public $PageObjName = "DocumentoSearch";
    ...
    {
        $srchUrl = "";
        $this->buildSearchUrl($srchUrl, $this->CodDocumento); // CodDocumento
        $this->buildSearchUrl($srchUrl, $this->Gestion); // Gestion
        $this->buildSearchUrl($srchUrl, $this->Codigo); // Codigo
    ...
    protected function loadSearchValues()
    {
        // Load search values
        $hasValue = false;
        if ($this->CodDocumento->AdvancedSearch->post()) {
            $hasValue = true;
        }
        if ($this->Gestion->AdvancedSearch->post()) {
            $hasValue = true;
        }
        if ($this->Codigo->AdvancedSearch->post()) {
            $hasValue = true;
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->Gestion->ViewValue = FormatNumber($this->Gestion->ViewValue, 0, -2, -2, -2);
            $this->Gestion->ViewCustomAttributes = "";

            // Codigo
            $this->Codigo->ViewValue = $this->Codigo->CurrentValue;
            $this->Codigo->ViewValue = FormatNumber($this->Codigo->ViewValue, 0, -2, -2, -2);
            ...
            $this->CodDocumento->EditValue = HtmlEncode($this->CodDocumento->AdvancedSearch->SearchValue);
            $this->CodDocumento->PlaceHolder = RemoveHtml($this->CodDocumento->caption());

            // Gestion
            $this->Gestion->EditAttrs["class"] = "form-control";
            $this->Gestion->EditCustomAttributes = "";
            $this->Gestion->EditValue = HtmlEncode($this->Gestion->AdvancedSearch->SearchValue);
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckInteger($this->CodDocumento->AdvancedSearch->SearchValue)) {
            $this->CodDocumento->addErrorMessage($this->CodDocumento->getErrorMessage(false));
        }
        if (!CheckInteger($this->Gestion->AdvancedSearch->SearchValue)) {
            $this->Gestion->addErrorMessage($this->Gestion->getErrorMessage(false));
        }
        if (!CheckInteger($this->Codigo->AdvancedSearch->SearchValue)) {
    ...
    {
        $this->CodDocumento->AdvancedSearch->load();
