 ...
 * Page class
 */
class ServicioEdit extends Servicio
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ServicioEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("CodServicio") ?? Key(0) ?? Route(2)) !== null) {
                $this->CodServicio->setQueryStringValue($keyValue);
                $this->CodServicio->setOldValue($this->CodServicio->QueryStringValue);
            } elseif (Post("CodServicio") !== null) {
                $this->CodServicio->setFormValue(Post("CodServicio"));
                $this->CodServicio->setOldValue($this->CodServicio->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("CodServicio") ?? Route("CodServicio")) !== null) {
                    $this->CodServicio->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("serviciolist"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "serviciolist") {
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['Sigla']); // Sigla
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("serviciolist"), "", $this->TableVar, true);
        $pageId = "edit";
