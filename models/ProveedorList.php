 ...
 * Page class
 */
class ProveedorList extends Proveedor
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ProveedorList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fproveedorlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "proveedoradd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "proveedordelete";
        $this->MultiUpdateUrl = "proveedorupdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fproveedorlistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->CodRegion->AdvancedSearch->toJson(), ","); // Field CodRegion
        $filterList = Concat($filterList, $this->Vigente->AdvancedSearch->toJson(), ","); // Field Vigente
        $filterList = Concat($filterList, $this->MinimoValidado->AdvancedSearch->toJson(), ","); // Field MinimoValidado
        $filterList = Concat($filterList, $this->tipo_transferencia->AdvancedSearch->toJson(), ","); // Field tipo_transferencia
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fproveedorlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field CodRegion
        $this->CodRegion->AdvancedSearch->SearchValue = @$filter["x_CodRegion"];
        $this->CodRegion->AdvancedSearch->SearchOperator = @$filter["z_CodRegion"];
        $this->CodRegion->AdvancedSearch->SearchCondition = @$filter["v_CodRegion"];
        $this->CodRegion->AdvancedSearch->SearchValue2 = @$filter["y_CodRegion"];
        $this->CodRegion->AdvancedSearch->SearchOperator2 = @$filter["w_CodRegion"];
        $this->CodRegion->AdvancedSearch->save();
        ...
        // Field MinimoValidado
        $this->MinimoValidado->AdvancedSearch->SearchValue = @$filter["x_MinimoValidado"];
        $this->MinimoValidado->AdvancedSearch->SearchOperator = @$filter["z_MinimoValidado"];
        $this->MinimoValidado->AdvancedSearch->SearchCondition = @$filter["v_MinimoValidado"];
        $this->MinimoValidado->AdvancedSearch->SearchValue2 = @$filter["y_MinimoValidado"];
        $this->MinimoValidado->AdvancedSearch->SearchOperator2 = @$filter["w_MinimoValidado"];
        $this->MinimoValidado->AdvancedSearch->save();

        // Field tipo_transferencia
        $this->tipo_transferencia->AdvancedSearch->SearchValue = @$filter["x_tipo_transferencia"];
        $this->tipo_transferencia->AdvancedSearch->SearchOperator = @$filter["z_tipo_transferencia"];
        $this->tipo_transferencia->AdvancedSearch->SearchCondition = @$filter["v_tipo_transferencia"];
        $this->tipo_transferencia->AdvancedSearch->SearchValue2 = @$filter["y_tipo_transferencia"];
        $this->tipo_transferencia->AdvancedSearch->SearchOperator2 = @$filter["w_tipo_transferencia"];
        $this->tipo_transferencia->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->CodRegion); // CodRegion
            $this->updateSort($this->Vigente); // Vigente
            $this->updateSort($this->MinimoValidado); // MinimoValidado
            $this->updateSort($this->tipo_transferencia); // tipo_transferencia
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->CodRegion->setSort("");
                $this->Vigente->setSort("");
                $this->MinimoValidado->setSort("");
                $this->tipo_transferencia->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if (IsMobile()) {
                    ...
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"proveedor\" data-caption=\"" . $viewcaption . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,url:'" . HtmlEncode(GetUrl($this->ViewUrl)) . "',btn:null});\">" . $Language->phrase("ViewLink") . "</a>";
                }
            } else {
                $opt->Body = "";
            }
            ...
            if ($Security->canEdit()) {
                if (IsMobile()) {
                    ...
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"proveedor\" data-caption=\"" . $editcaption . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SaveBtn',url:'" . HtmlEncode(GetUrl($this->EditUrl)) . "'});\">" . $Language->phrase("EditLink") . "</a>";
                }
            } else {
                $opt->Body = "";
            }
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->CodProveedor->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if (IsMobile()) {
            ...
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"proveedor\" data-caption=\"" . $addcaption . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'AddBtn',url:'" . HtmlEncode(GetUrl($this->AddUrl)) . "'});\">" . $Language->phrase("AddLink") . "</a>";
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fproveedorlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fproveedorlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fproveedorlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fproveedorlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
