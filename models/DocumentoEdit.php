 ...
 * Page class
 */
class DocumentoEdit extends Documento
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'documento';

    // Page object name
    public $PageObjName = "DocumentoEdit";
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["documento"]) || get_class($GLOBALS["documento"]) == PROJECT_NAMESPACE . "documento") {
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'documento');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("documento"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->CodDocumento->setVisibility();
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("CodDocumento") ?? Key(0) ?? Route(2)) !== null) {
                $this->CodDocumento->setQueryStringValue($keyValue);
                $this->CodDocumento->setOldValue($this->CodDocumento->QueryStringValue);
            } elseif (Post("CodDocumento") !== null) {
                $this->CodDocumento->setFormValue(Post("CodDocumento"));
                $this->CodDocumento->setOldValue($this->CodDocumento->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("CodDocumento") ?? Route("CodDocumento")) !== null) {
                    $this->CodDocumento->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->CodDocumento->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("documentolist"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = "documentolist";
                if (GetPageName($returnUrl) == "documentolist") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'CodDocumento' first before field var 'x_CodDocumento'
        $val = $CurrentForm->hasValue("CodDocumento") ? $CurrentForm->getValue("CodDocumento") : $CurrentForm->getValue("x_CodDocumento");
        if (!$this->CodDocumento->IsDetailKey) {
            $this->CodDocumento->setFormValue($val);
        }

        // Check field name 'Gestion' first before field var 'x_Gestion'
        $val = $CurrentForm->hasValue("Gestion") ? $CurrentForm->getValue("Gestion") : $CurrentForm->getValue("x_Gestion");
        if (!$this->Gestion->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->Gestion->Visible = false; // Disable update for API request
            } else {
                $this->Gestion->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->CodDocumento->CurrentValue = $this->CodDocumento->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->Gestion->EditValue = HtmlEncode($this->Gestion->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->CodDocumento->Required) {
            if (!$this->CodDocumento->IsDetailKey && EmptyValue($this->CodDocumento->FormValue)) {
                $this->CodDocumento->addErrorMessage(str_replace("%s", $this->CodDocumento->caption(), $this->CodDocumento->RequiredErrorMessage));
            }
        }
        if ($this->Gestion->Required) {
            if (!$this->Gestion->IsDetailKey && EmptyValue($this->Gestion->FormValue)) {
                $this->Gestion->addErrorMessage(str_replace("%s", $this->Gestion->caption(), $this->Gestion->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->Gestion->FormValue)) {
            $this->Gestion->addErrorMessage($this->Gestion->getErrorMessage(false));
        }
        ...
        if (!CheckInteger($this->Codigo->FormValue)) {
            $this->Codigo->addErrorMessage($this->Codigo->getErrorMessage(false));
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("documentolist"), "", $this->TableVar, true);
        $pageId = "edit";
